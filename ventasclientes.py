...
class Ventasclientes(ttk.Frame):
    db_name = "database.db"
    ...
    def on_visibility(self, event):
        if hasattr(self.master, 'controlador') and hasattr(self.master.controlador, 'current_client_name'):
            self.entry_cliente.config(state='normal')
            self.entry_cliente.delete(0, 'end')
            self.entry_cliente.insert(0, self.master.controlador.current_client_name)
    ...
    def procesar_pago(self, cantidad_pagada, ventana_pago, total_venta):
        ...
        try:
            ...
            self.mostrar_valoracion()
            
        except sqlite3.Error as e:
            messagebox.showerror("Error", f"Error al registrar la venta: {e}")
        
    def mostrar_valoracion(self):
        ventana_valoracion = tk.Toplevel(self)
        ventana_valoracion.title("Valoración del Servicio")
        ventana_valoracion.geometry("600x300+400+200")
        ventana_valoracion.configure(bg="white")
        ventana_valoracion.resizable(False, False)
        ventana_valoracion.transient(self.master)
        ventana_valoracion.grab_set()
        ventana_valoracion.focus_set()
        ventana_valoracion.lift()
        
        label_titulo = tk.Label(ventana_valoracion, text="¡Gracias por tu compra!", font="{Segoe UI} 18 bold", bg="white")
        label_titulo.place(x=150, y=30)
        
        label_sub = tk.Label(ventana_valoracion, text="¿Cómo calificarías nuestro servicio?", font="{Segoe UI} 16 bold", bg="white")
        label_sub.place(x=110, y=90)
        
        def enviar_valoracion(puntos):
            from utils import show_toast
            show_toast("Valoración", f"Has calificado con {puntos} estrellas. ¡Muchas gracias!")
            ventana_valoracion.destroy()
            if hasattr(self.master, 'controlador'):
                self.master.controlador.logout()
        ...
        def on_enter(event, idx):
            for i in range(5):
                if i <= idx:
                    stars[i].config(fg="#F2B04A")
                else:
                    stars[i].config(fg="#DDDDDD")
        ...
        def on_click(event, idx):
            enviar_valoracion(idx + 1)
            
        for i in range(5):
            lbl = tk.Label(ventana_valoracion, text="★", font="{Segoe UI} 50 bold", bg="white", fg="#DDDDDD", cursor="hand2")
            lbl.place(x=110 + i*80, y=150, width=70, height=70)
            lbl.bind("<Enter>", lambda e, idx=i: on_enter(e, idx))
            lbl.bind("<Leave>", on_leave)
            lbl.bind("<Button-1>", lambda e, idx=i: on_click(e, idx))
            stars.append(lbl)
    ...
    def widgets(self):
        ...
        self.entry_cliente = ttk.Entry(labelframe, font="sans 14 bold" )
